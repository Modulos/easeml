        ...
        for (let fileType in TYPE_EXTENSIONS) {
            for (let subtype in TYPE_EXTENSIONS[fileType]) {
                ...
                if (childName.endsWith(ext)) {
                    ...
                    if (fileType === "tensor") {
                        var loadResult = loader(root, dirPath, childName, opener, metadataOnly, subtype);
                        child = loadResult[0]
                        ex = loadResult[1]
                    } else {
...
function loadDataset(root, opener, metadataOnly=false) {
    let directories = loadDirectory(root, "", "", opener, metadataOnly);
...
function loadTensor(root, relPath, name, opener, metadataOnly=false, subtype="default") {
    ...
    if (subtype === "default") {
        ...
        return [new Tensor(name, shape, data), data]
