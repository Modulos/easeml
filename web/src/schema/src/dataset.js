...
function loadDirectory(root, relPath, name, opener, metadataOnly=false) {
    ...
    for (let i = 0; i < dirlist.length; i++) {
        ...
        if (childName.endsWith(TYPE_EXTENSIONS["tensor"])) {
            ...
            let loadTensorResult = loadTensor(root, dirPath, childName, opener, metadataOnly);
            child = loadTensorResult[0];
            ex = loadTensorResult[1];
            ...
            let loadResult = loadDirectory(root, dirPath, childName, opener, metadataOnly);
            child = loadResult[0];
            ex = loadResult[1];
        }

        children[childName] = child;
        examples[childName] = ex;

    }

    return [new Directory(name, children), new Directory(name, examples)];
}
...
function loadDataset(root, opener, metadataOnly=false) {
    //let loadDirOutput = loadDirectory(root, "", "", opener, metadataOnly);
    var directories = loadDirectory(root, "", "", opener, metadataOnly);
    var outputdir1 = new Dataset(root, directories[0])
    var outputdir2 = new Dataset(root, directories[1])
    return [outputdir1, outputdir2]
}
...
function loadTensor(root, relPath, name, opener, metadataOnly=false) {
    ...
    return [new Tensor(name, npyReader.shape, data), data];
}
